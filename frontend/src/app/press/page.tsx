...
const YourComponent: NextPage = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/press");
        ...
        {data
          ...
          .map((item) => (
            <div key={item.id} className="border border-gray-300 rounded">
              ...
              >
                <div className="font-semibold text-teal-800 flex-col">
                  {item.title}
                </div>
                <div className="text-sm text-rose-800 flex-col">
                  {item.publication_date}
                </div>
              </button>
              {openItemId === item.id && (
                <div className="grid items-center grid-cols-3  grid-flow-col mx-6 pb-5">
                  {item.image && (
                    <div className="mt-4 ">
                      <Image
                        src={item.image}
                  ...
                  <p className="text-gray-700 col-span-4">{item.content}</p>
                </div>
              )}
            </div>
          ))}
