...
// //     const [flightData, setFlightData] = useState([]); // Add state for flight data [flightData, setFlightData
...
// //         try {
// //             const response = await axios.get("http://127.0.0.1:8000/passengers/");
...
interface Passenger {
    ...
    last_name: string;
    gender: string;
    user: number; // Assuming it's the user ID
}
...
export default function Page() {
    ...
    const [userData, setUserData] = useState([]);
    const [passengerData, setPassengerData] = useState([]);
    const [flightData, setFlightData] = useState<any[]>([]); // Adjust type as per your flight data structure
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState("");
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {bookingData.map((booking, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
    {Array.isArray(flightData) && flightData.find((flight) => flight.id === booking.flight)?.flight_number}
</td>
                                {/* Display other booking details in table cells */}
                            </tr>
                        ))}
