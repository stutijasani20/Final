...
<div className="container mx-auto p-6 bg-slate-200 rounded-lg shadow-lg">
  <h1 className="text-3xl font-bold mb-4 text-sky-900 text-center">Explore Hotels</h1>
  ...
  {loading ? (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {hotels.map((hotel, index) => (
        ...
        >
         <h2 className="text-xl text-sky-800 font-semibold mb-2">{hotel.name}</h2>
          <hr className="mb-2"/>
          {hotel.phone && (
          ...
          <p className="mb-2 text-sky-600">
            <FaMapMarkerAlt className="inline-block" /> Address: {hotel.address.streetNumber}{" "}
            {hotel.address.streetName}, {hotel.address.municipality}, {hotel.address.postalCode},{" "}
