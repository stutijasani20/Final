...
import { FiMapPin, FiClock, FiCalendar, FiMap } from 'react-icons/fi'; // Importing specific icons
...
const MapWithRoute = () => {
  ...
  useEffect(() => {
    const fetchRouteData = async () => {
      try {
        const restaurantLat = parseFloat(searchParams.get("restaurantLat") || "");
        const restaurantLng = parseFloat(searchParams.get("restaurantLng") || "");
    ...
    if (searchParams.has("restaurantLat") && searchParams.has("restaurantLng")) {
      fetchRouteData();
    }
          ...
          <div className="max-w-screen-lg mx-auto bg-white rounded-lg shadow-md p-8">
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="text-gray-500">
                <p className="font-semibold"><FiMapPin className="inline mr-1" /> From: {airport}</p>
                <p className="font-semibold"><FiMap className="inline mr-1" /> To: Restaurant</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="border border-gray-200 rounded-lg p-6">
                <h2 className="text-xl font-semibold mb-4">Summary</h2>
                <p className="text-gray-700 mb-2"><FiClock  className="inline mr-1 text-red-600" /> Travel Time: {secondsToHoursMinutes(routeData.routes[0].summary.travelTimeInSeconds)}</p>
                <p className="text-gray-700 mb-2">Distance: {metersToKilometers(routeData.routes[0].summary.lengthInMeters)} km</p>
                <p className="text-gray-700 mb-2"><FiCalendar className="inline mr-1" /> Departure Time: {formatDate(routeData.routes[0].summary.departureTime)}</p>
            ...
            <div className="mt-8">
              <MapContainer center={[userLocation.lat, userLocation.lng]} zoom={16} className="h-96">
                ...
                <Marker position={[restaurantLocation.lat, restaurantLocation.lng]} icon={markerIcon2}>
